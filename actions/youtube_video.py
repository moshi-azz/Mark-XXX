...
def open_browser(url=None):
    if url:
        ...
        # Fallback to opening just a browser if no URL is provided
...
def _get_transcript(video_id: str) -> str | None:
    ...
    try:
        ...
        print(f"[YouTube] [INFO] Transcript: {len(text)} chars")
        return text
...
def _save_to_notepad(content: str, video_url: str) -> str:
    ...
    print(f"[YouTube] [OK] Summary saved: {filepath}")
...
def _handle_play(parameters: dict, player, speak=None) -> str:
    ...
    if player:
        player.write_log(f"[YouTube] Opening: {url}")
    ...
    if len(thumbnails) >= 2:
        x, y = thumbnails[1]
        print(f"[YouTube] [OK] Clicking 2nd thumbnail at ({x}, {y})")
        pyautogui.click(x, y)
